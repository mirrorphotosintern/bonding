# Handful — Product Specification

**Status:** Concept specification  
**Internal codename:** Bonding  
**Recommended working name:** Handful  

> **Global product boundary:** Handful is a standalone product for families worldwide. It has no Kannada, Indian, diaspora, or language-learning positioning. International games may appear when they are independently understandable, safely adapted, properly attributed, and enjoyable outside their culture of origin.
...
The sibling `/connectplay` project is a working static idea bank with 74 cards: 44 founder-saved/source-linked records and 30 researched additions. It validates provenance browsing and basic filtering, but it is not the Handful product described here. Handful should migrate qualified content while preserving raw notes and URLs; it should not port ConnectPlay's many-card grid into the one-match Today experience. See `CONNECTPLAY_IMPLEMENTATION_AUDIT.md`.

## 1. Product summary

Handful helps a tired or time-poor caregiver find one safe, low-prep thing to do with a child right now. It asks only what matters in the moment—who is here, time, place, energy, and available materials—then recommends one shared challenge.

The product is intentionally designed to be exited. After a short preparation screen, **Together Mode** locks the activity into a glanceable or audio-led sequence, suppresses browsing, and lets the adult put the phone face down. The completion is a tiny shared reflection, not a child score or a parenting grade.

The fastest version of this promise is **Talk Now**: a zero-prop conversation game chosen for a car ride, restaurant wait, queue, or bedtime. Its rule is explainable in one breath, the phone can go down within 15 seconds, and the family can stop cleanly whenever the real-world wait ends.

Handful is inspired by Hearty's useful premise—age-tailored offline bonding—but it is not a Hearty clone. Its defensible product idea is a **context-to-completion system**:
...
- Child-only use: Handful is adult-operated
...
Tell Handful where you are, how much time you have, and what energy is left. Get one safe, age-fit activity to do with your child—then put the phone down.

## 7. Core product loop

```mermaid
flowchart LR
    A["Open Handful"] --> B["Set the moment<br/>who · time · place · energy"]
    B --> C["One matched activity"]
    C --> D{"Works for us?"}
    D -- "Swap" --> C
    D -- "Yes" --> E["30-second prep + safety"]
    E --> F["Together Mode<br/>glanceable or audio-led"]
    F --> G["Do it away from the screen"]
    G --> H["Tiny shared close"]
    H --> I["Preference signal improves next match"]
```
...
1. **Today** — one matched activity and fast moment controls.
2. **Explore** — deliberate browsing by mode, skill, material, or situation.
3. **Our Things** — favorites, downloaded packs, family-made variations, and recent hits.
4. **Grown-ups** — child profiles, co-caregivers, reminders, subscription, privacy, and help.

Together Mode is a full-screen route outside the tab hierarchy.

### Why four tabs

- Today protects the primary one-answer interaction.
- Explore handles weekend planning without contaminating Today with a feed.
- Our Things turns successful activities into family culture.
- Grown-ups keeps administration away from the child-facing moment.
...
Today always offers **Talk Now — no materials** beneath the primary match. It asks only whether the family is in the car, waiting somewhere, or winding down. Age and participant count come from the active family profile.
...
Together Mode is Handful's signature feature.

For conversation games, Together Mode collapses further into **one-prompt mode**. It supplies the rule and opening line, then disappears; it does not add steps or a timer unless the game genuinely requires them.

### Entry ritual

1. “Everything ready?”
2. Optional “Turn on voice guide.”
3. A deliberate two-second transition:
   - screen simplifies;
   - nonessential controls disappear;
   - system brightness may reduce;
   - the prompt says “Put me down. I’ll tell you when you need me.”

The app cannot programmatically guarantee that the phone is face down on every platform, so this is a behavioral design pattern, not a false technical claim.

### Modes

- **Face-down audio:** short locally cached cues and timer sounds.
- **Glance mode:** large text and diagrams for activities that require reference.
- **One-card mode:** all rules fit on a single static screen.
- **Turn mode:** a large shared prompt changes only when someone taps “Next turn.”
- **One-prompt talk:** one-breath rule plus opening line; no mid-game prompts.
...
The source file includes an idea where AI judges faces. Handful should preserve the playful “make a face from an adjective” challenge but make the family choose the funniest, clearest, or most surprising interpretation. A machine should not evaluate children's appearance.
...
1. the adult starts Together Mode;
...
- First Together Mode start within first session
...
- Median in-app time before Together Mode should decline with familiarity
...
- Talk Now for car, waiting, and winding-down contexts
- Today recommendation
- Swap with reasons
- Activity prep
- Together Mode: one-card, glance, timer/audio cue, and one-prompt talk
...
- 120 activities
- Cloudflare D1/R2 sync through a Handful Worker
- Handful adult accounts using Apple/Google identity verification
- RevenueCat
- Multiple children
- Downloads
...
| --- | --- | --- |
| Crowded category | Several 2026 apps already promise one low-prep idea | Own Together Mode, multi-age roles, skill paths, and context-to-completion quality |
...
| Name collision | “Handful” is used by an unrelated app | Treat as working name; complete clearance before code or public assets |
...
7. Is **Handful** worth clearing, or should naming be a separate sprint?
